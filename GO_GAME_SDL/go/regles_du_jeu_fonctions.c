...
int data[81],lib[81],interdit[81],check[81],ami[81],check2[81];
int old[81],old2[81],old3[81];
int in=0,player=0,ko=0,end_game=0;
float player1_score=0,player2_score=6.5;
int player1_territory=0,player2_territory=0,black_stones=0,white_stones=0;
int terr[81],terr2[81],terr3[81];
    ...
    int compt=0;
            ...
            for(int i=0;i<81;i++)
            {
                if(check[i]==0)
                {
                    ...
                    for(int j=0;j<81;j++)
                    {
                        if(check[j]==1&&check2[j]==0) //check2 contient ceux qu'on a deja attribué leurs degré de liberté
                        {
                            lib[j]=compt;
                            check2[j]=1;
                            if(compt==0&&j!=in&&data[j]!=data[in]) // pour qu'il ne sera capturé que ceux qui entourne in, ce dernier et ces amis non
                            {
                                check2[j]=2;
                            }
                        }
...
void zone()
{
    ...
    for(int m=0;m<81;m++)
    {
        ...
        if(lib[m]!=0)
        {
            ...
            //maintenant le traitement du cas ou le groupe ne doit pas etre capture
